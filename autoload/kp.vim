...
" =================
function! kp#Stamp()
   " get hour and format into time stamp
   silent! let l:stamp = strftime("|%H:00|")
   let l:res = 0
   " check if the current hour has already been stamped
   exe "g/" . l:stamp . "/let l:res = l:res+1"
   if (l:res == 0)
      " paste time stamp one line below previous EOF
      normal Go
      call kp#padAbove()
      exe "normal 80i-\<Esc>"
      $pu=l:stamp
      normal G
      right 80        " right justified
      call kp#padBelow()
      normal G
      " exe "normal o\<Esc>d0"
      left
